ig.module(
	...
	.defines(function () {
		...
		var MyGame = ig.GameExtended.extend({
			...
			onPlayerCollectedWeapon: function (item) {
				this.connection.broadcastMessage(MessageBuilder.createMessage(MESSAGE_COLLECT_WEAPON).setWeaponId(item.itemId));
				this.onWeaponCollected(item.itemId);
			},



			onWeaponCollected: function (itemId) {
			...
			checkItemTimer: function () {
				...
				if (this.timerItem && this.timerItem.delta() > 0) {
					while (this.itemQueue.length > 0) {
						console.log("queue: " + this.itemQueue.length);
			...
			// 			ig.game.spawnEntity(entity, positionSet[i][0], positionSet[i][1], { itemId:  });
			// 		}
			// 	}
			// },
